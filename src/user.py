...
def user_profile_v1(token, u_id):
    ...
    if user == None:
        raise InputError(desciption="Invalid u_id")
...
def user_profile_setemail_v1(token, email):
    ...
    for user_index in range(len(store['users'])):
        if store['users'][user_index]['id'] == u_id:
            if store['users'][user_index]['email'] == email:
...
def user_profile_sethandle_v1(token, handle):
    ...
    for user_index in range(len(store['users'])):
        if store['users'][user_index]['id'] == u_id:
            if store['users'][user_index]['handle_str'] == handle:
                raise InputError ("handle_str is the same as previous")
