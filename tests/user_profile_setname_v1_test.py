...
def test_invalid_token_and_valid_name(clear_and_register_user0):
    ...
    assert requests.put(config.url + '/user/profile/setname/v1/', data=input).status_code == AccessError.code
...
def test_valid_token_and_invalid_name(clear_and_register_user0):
    ...
    for first_name in first:
        ...
        for last_name in last:
            ...
            assert requests.put(config.url + '/user/profile/setname/v1/', data=input).status_code == InputError.code
...
def test_same_name_with_others(clear_and_register_user0):
    ...
    requests.post(config.url + '/auth/register/v2', data=user1_register).json()
    input = {
        "token": user0['token'],
        "name_first": "firstname1",
        "name_last": "lastname1"
    }
    requests.put(config.url + '/user/profile/setname/v1/', data=input)
    profile_user0 = requests.get(config.url + '/user/profile/v1/', params=input).json()
    assert profile_user0["user"]["name_first"] == "firstname1"
    assert profile_user0["user"]["name_last"] == "lastname1"
