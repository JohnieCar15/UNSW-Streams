...
def test_valid_channel_authorised(clear_and_register):
    ...
    channel_leave = requests.post(config.url + 'channel/leave/v1', params={'token': token, 'channel_id': channel_id})
    channel_leave_data = channel_leave.json()
    assert channel_leave_data == {}
    channel_details = requests.get(config.url + 'channel/details/v1', params={'token': token, 'channel_id': channel_id})
...
def test_valid_channel_unauthorised(clear_and_register):
    ...
    channel_leave = requests.post(config.url + 'channel/leave/v1', params={'token': token_2, 'channel_id': channel_id})
...
def test_valid_channel_invalid_token(clear_and_register):
    token = clear_and_register['token']
    channel_id = clear_and_register['channel_id']
    channel_leave = requests.post(config.url + 'channel/leave/v1', params={'token': token + 1, 'channel_id': channel_id})
    assert channel_leave.status_code == AccessError.code
...
def test_invalid_channel_valid_token():
    ...
    channel_leave = requests.post(config.url + 'channel/leave/v1', params={'token': token, 'channel_id': 1})
...
def test_invalid_channel_invalid_token():
    requests.delete(config.url + 'clear/v2')
    channel_leave = requests.post(config.url + 'channel/leave/v1', params]{'token': 1, 'channel_id': 1})
    assert channel_leave.status_code == AccessError.code
